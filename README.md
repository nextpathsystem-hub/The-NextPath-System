<!DOCTYPE html>
<html lang="fr" id="html-root">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>NextPath System</title>
<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;0,900;1,400;1,700&family=Montserrat:wght@300;400;500;600;700&display=swap" rel="stylesheet">
<style>
:root {
  --navy: #0D1B3E;
  --gold: #C9A84C;
  --cream: #F5F0E8;
  --white: #FFFFFF;
  --light-bg: #F8F7F4;
  --text-dark: #1A1A2E;
  --text-mid: #4A4A6A;
  --text-light: #8A8AAA;
  --border: #E8E4DC;
  --gold-light: rgba(201,168,76,0.12);
  --navy-light: rgba(13,27,62,0.06);
}
* { margin:0; padding:0; box-sizing:border-box; }
html { scroll-behavior:smooth; }
body { background:#fff; color:var(--text-dark); font-family:'Montserrat',sans-serif; font-size:15px; line-height:1.7; }

/* ── NAV ── */
nav {
  position:fixed; top:0; left:0; right:0; z-index:200;
  background:rgba(255,255,255,0.97);
  backdrop-filter:blur(12px);
  border-bottom:1px solid var(--border);
  padding:0 40px; height:70px;
  display:flex; align-items:center; justify-content:space-between;
}
.nav-logo { display:flex; align-items:center; gap:10px; text-decoration:none; flex-shrink:0; }
.nav-logo img { height:36px; width:auto; }
.nav-logo span { font-family:'Playfair Display',serif; font-size:16px; font-weight:700; color:var(--navy); }
.nav-links { display:flex; gap:0; list-style:none; align-items:center; }
.nav-links a {
  font-size:11px; font-weight:500; letter-spacing:.07em; text-transform:uppercase;
  color:var(--text-mid); text-decoration:none; padding:0 12px; height:70px;
  display:flex; align-items:center; border-bottom:2px solid transparent; transition:all .2s;
}
.nav-links a:hover { color:var(--navy); border-bottom-color:var(--gold); }
.nav-cta {
  background:var(--navy) !important; color:#fff !important;
  border-bottom-color:transparent !important;
  padding:9px 18px !important; height:auto !important;
  border-radius:2px; margin-left:8px;
}
.nav-cta:hover { background:var(--gold) !important; color:var(--navy) !important; }

/* LANG SWITCHER */
.lang-switcher {
  display:flex; gap:4px; align-items:center; margin-left:8px;
}
.lang-btn {
  font-size:11px; font-weight:600; padding:4px 8px; border-radius:2px;
  cursor:pointer; border:1px solid var(--border); background:transparent;
  color:var(--text-light); letter-spacing:.06em; transition:all .15s;
}
.lang-btn.active { background:var(--navy); color:#fff; border-color:var(--navy); }

/* HAMBURGER */
.hamburger {
  display:none; flex-direction:column; gap:5px; cursor:pointer;
  padding:8px; background:none; border:none;
}
.hamburger span {
  display:block; width:22px; height:2px; background:var(--navy); border-radius:1px; transition:all .3s;
}
.mobile-menu {
  display:none; position:fixed; top:70px; left:0; right:0; z-index:199;
  background:#fff; border-bottom:1px solid var(--border);
  padding:16px 24px; flex-direction:column; gap:4px;
  box-shadow:0 8px 24px rgba(0,0,0,0.08);
}
.mobile-menu.open { display:flex; }
.mobile-menu a {
  font-size:13px; font-weight:500; letter-spacing:.06em; text-transform:uppercase;
  color:var(--text-dark); text-decoration:none; padding:12px 0;
  border-bottom:1px solid var(--border); display:block;
}
.mobile-menu a:last-child { border-bottom:none; }

/* PAGES */
.page { min-height:100vh; padding-top:70px; display:none; }
.page.active { display:block; }

/* HERO */
.hero {
  min-height:calc(100vh - 70px);
  display:grid; grid-template-columns:1fr 1fr;
  align-items:stretch; overflow:hidden;
}
.hero-left {
  padding:80px 60px 80px 80px;
  display:flex; flex-direction:column; justify-content:center;
}
.hero-tag {
  display:inline-flex; align-items:center; gap:10px;
  font-size:11px; font-weight:600; letter-spacing:.18em; text-transform:uppercase;
  color:var(--gold); margin-bottom:28px;
}
.hero-tag::before { content:''; display:block; width:28px; height:1.5px; background:var(--gold); }
.hero h1 {
  font-family:'Playfair Display',serif;
  font-size:clamp(32px,4vw,56px); font-weight:900; line-height:1.1;
  color:var(--navy); margin-bottom:20px;
}
.hero h1 em { color:var(--gold); font-style:italic; }
.hero-sub { font-size:15px; color:var(--text-mid); line-height:1.8; margin-bottom:40px; max-width:460px; }
.hero-actions { display:flex; gap:14px; flex-wrap:wrap; margin-bottom:48px; }
.btn-primary {
  display:inline-flex; align-items:center; gap:8px;
  background:var(--navy); color:#fff;
  font-size:11px; font-weight:600; letter-spacing:.1em; text-transform:uppercase;
  padding:14px 28px; border-radius:2px; text-decoration:none;
  transition:all .25s; border:2px solid var(--navy);
}
.btn-primary:hover { background:var(--gold); border-color:var(--gold); color:var(--navy); }
.btn-secondary {
  display:inline-flex; align-items:center; gap:8px;
  border:2px solid var(--navy); color:var(--navy);
  font-size:11px; font-weight:600; letter-spacing:.1em; text-transform:uppercase;
  padding:14px 28px; border-radius:2px; text-decoration:none; transition:all .25s;
}
.btn-secondary:hover { background:var(--navy); color:#fff; }
.btn-gold {
  display:inline-flex; align-items:center; gap:8px;
  background:var(--gold); color:var(--navy);
  font-size:11px; font-weight:700; letter-spacing:.1em; text-transform:uppercase;
  padding:14px 28px; border-radius:2px; text-decoration:none;
  transition:all .25s; border:2px solid var(--gold);
}
.btn-gold:hover { background:var(--navy); border-color:var(--navy); color:#fff; }
.hero-quote {
  padding:22px 26px; border-left:3px solid var(--gold);
  background:var(--light-bg); border-radius:0 4px 4px 0; max-width:420px;
}
.hero-quote p {
  font-family:'Playfair Display',serif; font-size:15px; font-style:italic;
  color:var(--navy); line-height:1.65; margin-bottom:8px;
}
.hero-quote cite { font-size:11px; letter-spacing:.1em; text-transform:uppercase; color:var(--gold); }
.hero-right {
  background:var(--navy);
  display:flex; align-items:center; justify-content:center; padding:60px 48px;
}
.hero-right-content { text-align:center; }
.hero-right-content .line { width:48px; height:1.5px; background:rgba(201,168,76,0.4); margin:24px auto; }
.hero-right-content h2 {
  font-family:'Playfair Display',serif; font-size:clamp(22px,2.5vw,32px);
  font-weight:700; color:var(--gold); line-height:1.3; letter-spacing:.02em;
}

/* STATS STRIP */
.stats-strip { background:var(--navy); border-top:3px solid var(--gold); padding:52px 0; }
.stats-inner { max-width:900px; margin:0 auto; display:flex; align-items:center; justify-content:center; }
.stat-col { flex:1; text-align:left; padding:0 48px; border-right:1px solid rgba(201,168,76,0.2); }
.stat-col:last-child { border-right:none; }
.stat-col:first-child { padding-left:0; }
.stat-num { font-family:'Playfair Display',serif; font-size:48px; font-weight:900; color:var(--gold); display:block; line-height:1; }
.stat-lbl { font-size:11px; color:rgba(245,240,232,0.6); letter-spacing:.08em; text-transform:uppercase; margin-top:8px; display:block; }

/* SECTIONS */
.section { padding:96px 80px; }
.section-alt { background:var(--light-bg); }
.container { max-width:1140px; margin:0 auto; }
.section-tag {
  display:inline-flex; align-items:center; gap:10px;
  font-size:11px; font-weight:600; letter-spacing:.18em; text-transform:uppercase;
  color:var(--gold); margin-bottom:16px;
}
.section-tag::before { content:''; display:block; width:24px; height:1.5px; background:var(--gold); }
.section-title {
  font-family:'Playfair Display',serif; font-size:clamp(26px,3.5vw,44px);
  font-weight:700; color:var(--navy); line-height:1.15; margin-bottom:20px;
}
.section-title em { color:var(--gold); font-style:italic; }
.section-body { font-size:15px; color:var(--text-mid); line-height:1.85; max-width:580px; }
.divider { width:48px; height:2px; background:var(--gold); margin:24px 0; opacity:.7; }

/* PILLARS */
.pillars-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:24px; margin-top:48px; }
.pillar-card {
  background:#fff; border:1px solid var(--border); border-radius:4px;
  padding:40px 32px; transition:all .3s; border-top:3px solid transparent;
}
.pillar-card:hover { border-top-color:var(--gold); box-shadow:0 8px 32px rgba(13,27,62,0.08); transform:translateY(-4px); }
.pillar-num { font-family:'Playfair Display',serif; font-size:52px; font-weight:900; color:var(--gold); line-height:1; margin-bottom:16px; }
.pillar-title { font-family:'Playfair Display',serif; font-size:20px; font-weight:700; color:var(--navy); margin-bottom:12px; text-transform:uppercase; letter-spacing:.04em; }
.pillar-text { font-size:14px; color:var(--text-mid); line-height:1.8; }

/* YOUTUBE SECTION */
.yt-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:20px; margin-top:40px; }
.yt-card {
  border:1px solid var(--border); border-radius:4px; overflow:hidden;
  transition:all .25s; text-decoration:none; display:block; background:#fff;
}
.yt-card:hover { border-color:var(--gold); box-shadow:0 8px 24px rgba(13,27,62,0.1); transform:translateY(-3px); }
.yt-thumb {
  position:relative; aspect-ratio:16/9; overflow:hidden; background:var(--navy);
  display:flex; align-items:center; justify-content:center;
}
.yt-thumb-bg {
  position:absolute; inset:0; background:linear-gradient(135deg, #0D1B3E 0%, #1a2d5a 100%);
}
.yt-play {
  position:relative; z-index:1; width:48px; height:48px; border-radius:50%;
  background:rgba(201,168,76,0.9); display:flex; align-items:center; justify-content:center;
  transition:transform .2s;
}
.yt-card:hover .yt-play { transform:scale(1.1); }
.yt-play::after {
  content:''; border-style:solid; border-width:8px 0 8px 16px;
  border-color:transparent transparent transparent var(--navy);
  margin-left:3px;
}
.yt-info { padding:16px; }
.yt-title { font-size:13px; font-weight:600; color:var(--text-dark); line-height:1.5; margin-bottom:6px; }
.yt-meta { font-size:11px; color:var(--text-light); letter-spacing:.04em; }

/* PROGRAMMES */
.prog-grid { display:grid; grid-template-columns:1fr 1fr; gap:24px; margin-top:40px; }
.prog-card {
  border:1px solid var(--border); border-radius:4px; padding:36px;
  transition:all .25s; cursor:pointer; position:relative; overflow:hidden;
}
.prog-card::before {
  content:''; position:absolute; top:0; left:0; right:0; height:3px;
  background:var(--gold); transform:scaleX(0); transition:transform .3s;
}
.prog-card:hover { border-color:var(--gold); box-shadow:0 8px 24px rgba(13,27,62,0.08); }
.prog-card:hover::before { transform:scaleX(1); }
.prog-icon { font-size:32px; margin-bottom:16px; display:block; }
.prog-name { font-family:'Playfair Display',serif; font-size:22px; font-weight:700; color:var(--navy); margin-bottom:10px; }
.prog-desc { font-size:13px; color:var(--text-mid); line-height:1.7; margin-bottom:20px; }
.prog-badge {
  display:inline-block; font-size:10px; font-weight:700; letter-spacing:.1em;
  text-transform:uppercase; padding:4px 10px; border-radius:20px;
  background:var(--gold-light); color:var(--gold); margin-bottom:16px;
}

/* MODULE */
.module-list { display:flex; flex-direction:column; gap:16px; }
.module-item {
  display:flex; gap:20px; align-items:flex-start;
  padding:20px 24px; border:1px solid var(--border); border-radius:4px; transition:all .25s;
}
.module-item:hover { border-color:var(--gold); background:var(--gold-light); }
.module-week {
  font-size:10px; font-weight:700; letter-spacing:.12em; text-transform:uppercase;
  color:var(--gold); background:var(--gold-light);
  padding:4px 10px; border-radius:20px; white-space:nowrap; flex-shrink:0; margin-top:3px;
}
.module-item h4 { font-family:'Playfair Display',serif; font-size:16px; font-weight:700; color:var(--navy); margin-bottom:6px; }
.module-item p { font-size:13px; color:var(--text-mid); line-height:1.7; }
.info-row { display:flex; justify-content:space-between; align-items:center; padding:14px 0; border-bottom:1px solid var(--border); font-size:14px; }
.info-row:last-child { border-bottom:none; }
.info-row span:first-child { color:var(--text-light); font-size:11px; letter-spacing:.08em; text-transform:uppercase; }
.info-row span:last-child { color:var(--text-dark); font-weight:500; }
.benefit-item { display:flex; gap:12px; align-items:flex-start; font-size:14px; color:var(--text-mid); line-height:1.6; margin-bottom:10px; }
.benefit-dot { width:20px; height:20px; border-radius:50%; background:var(--gold-light); border:1px solid var(--gold); display:flex; align-items:center; justify-content:center; flex-shrink:0; margin-top:2px; font-size:10px; color:var(--gold); }
.programme-cta { margin-top:40px; padding:32px; background:var(--navy); border-radius:4px; text-align:center; }
.programme-cta h3 { font-family:'Playfair Display',serif; font-size:22px; color:var(--gold); margin-bottom:10px; }
.programme-cta p { font-size:14px; color:rgba(245,240,232,0.7); margin-bottom:22px; line-height:1.7; }
.gratuit-badge { display:inline-block; background:var(--gold); color:var(--navy); font-size:11px; font-weight:700; letter-spacing:.1em; text-transform:uppercase; padding:4px 12px; border-radius:20px; margin-bottom:14px; }

/* PROFILES */
.profiles-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin:48px 0; }
.profile-card { padding:28px 20px; border:1px solid var(--border); border-radius:4px; text-align:center; transition:all .25s; }
.profile-card:hover { border-color:var(--gold); background:var(--gold-light); }
.profile-icon { font-size:28px; margin-bottom:12px; display:block; }
.profile-name { font-family:'Playfair Display',serif; font-size:16px; font-weight:700; color:var(--navy); margin-bottom:8px; }
.profile-desc { font-size:12px; color:var(--text-light); line-height:1.7; }

/* PDF / RESSOURCES */
.pdf-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:20px; margin-top:40px; }
.pdf-card {
  border:1px solid var(--border); border-radius:4px; padding:28px 24px;
  display:flex; flex-direction:column; gap:12px; transition:all .25s;
  text-decoration:none; background:#fff;
}
.pdf-card:hover { border-color:var(--gold); box-shadow:0 6px 20px rgba(13,27,62,0.08); transform:translateY(-2px); }
.pdf-icon {
  width:44px; height:44px; border-radius:8px; background:var(--navy);
  display:flex; align-items:center; justify-content:center; font-size:18px;
  flex-shrink:0;
}
.pdf-title { font-size:14px; font-weight:600; color:var(--text-dark); line-height:1.4; }
.pdf-desc { font-size:12px; color:var(--text-light); line-height:1.6; }
.pdf-dl {
  display:inline-flex; align-items:center; gap:6px; font-size:11px; font-weight:600;
  letter-spacing:.08em; text-transform:uppercase; color:var(--gold); margin-top:4px;
}
.pdf-dl::before { content:'↓'; font-size:14px; }

/* AVIS */
.avis-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:24px; margin-bottom:56px; }
.avis-card { background:#fff; border:1px solid var(--border); border-radius:4px; padding:32px; border-top:3px solid var(--gold); }
.stars { color:var(--gold); font-size:14px; margin-bottom:14px; }
.avis-text { font-size:14px; color:var(--text-mid); line-height:1.8; margin-bottom:18px; font-style:italic; }
.avis-author { display:flex; align-items:center; gap:12px; }
.avis-avatar { width:38px; height:38px; border-radius:50%; background:var(--navy); display:flex; align-items:center; justify-content:center; font-family:'Playfair Display',serif; font-weight:700; color:var(--gold); font-size:13px; flex-shrink:0; }
.avis-name { font-size:13px; font-weight:600; color:var(--navy); }
.avis-loc { font-size:11px; color:var(--text-light); letter-spacing:.06em; text-transform:uppercase; }

/* GRACE */
.grace-grid { display:grid; grid-template-columns:1fr 1fr; gap:80px; align-items:center; }
.grace-photo-wrap { position:relative; border-radius:4px; overflow:hidden; aspect-ratio:4/5; box-shadow:0 20px 60px rgba(13,27,62,0.15); }
.grace-photo-wrap img { width:100%; height:100%; object-fit:cover; object-position:center top; }
.grace-photo-wrap::after { content:''; position:absolute; inset:0; border:3px solid var(--gold); border-radius:4px; pointer-events:none; opacity:.4; }
.tagline-box { margin-top:28px; padding:22px 26px; border-left:3px solid var(--gold); background:var(--light-bg); }
.tagline-box p { font-family:'Playfair Display',serif; font-size:16px; font-style:italic; color:var(--navy); line-height:1.65; }

/* CONTACT */
.social-link { display:flex; align-items:center; gap:16px; padding:14px 18px; border:1px solid var(--border); border-radius:4px; text-decoration:none; transition:all .2s; color:var(--text-dark); margin-bottom:10px; }
.social-link:hover { border-color:var(--navy); background:var(--navy-light); }
.social-platform { font-size:10px; font-weight:700; letter-spacing:.12em; text-transform:uppercase; color:var(--gold); min-width:70px; }
.social-handle { font-size:14px; font-weight:500; color:var(--text-dark); }

/* FOOTER */
footer { background:var(--navy); padding:40px 80px; display:flex; justify-content:space-between; align-items:center; border-top:3px solid var(--gold); }
.footer-logo { font-family:'Playfair Display',serif; font-size:18px; font-weight:700; color:var(--gold); }
.footer-links { display:flex; gap:20px; flex-wrap:wrap; }
.footer-links a { font-size:11px; color:rgba(245,240,232,0.5); text-decoration:none; letter-spacing:.06em; transition:color .2s; }
.footer-links a:hover { color:var(--gold); }
.footer-copy { font-size:11px; color:rgba(245,240,232,0.3); letter-spacing:.06em; }

/* ── RESPONSIVE MOBILE ── */
@media(max-width:768px) {
  nav { padding:0 20px; }
  .nav-links { display:none; }
  .hamburger { display:flex; }
  .hero { grid-template-columns:1fr; min-height:auto; }
  .hero-left { padding:40px 24px; }
  .hero-right { min-height:220px; padding:40px 24px; }
  .hero-right-content h2 { font-size:22px; }
  .stats-strip { padding:40px 24px; }
  .stats-inner { flex-direction:column; gap:28px; max-width:100%; padding:0 24px; }
  .stat-col { border-right:none; border-bottom:1px solid rgba(201,168,76,0.2); padding:0 0 24px; text-align:center; }
  .stat-col:last-child { border-bottom:none; padding-bottom:0; }
  .section { padding:56px 24px; }
  .pillars-grid { grid-template-columns:1fr; }
  .yt-grid { grid-template-columns:1fr; }
  .prog-grid { grid-template-columns:1fr; }
  .profiles-grid { grid-template-columns:1fr 1fr; }
  .pdf-grid { grid-template-columns:1fr; }
  .avis-grid { grid-template-columns:1fr; }
  .grace-grid { grid-template-columns:1fr; gap:32px; }
  footer { flex-direction:column; gap:16px; text-align:center; padding:32px 24px; }
  .footer-links { justify-content:center; }
  .hero-actions { flex-direction:column; }
  .btn-primary, .btn-secondary, .btn-gold { justify-content:center; text-align:center; }
}
</style>
</head>
<body>

<!-- ══ NAV ══ -->
<nav>
  <a href="#" class="nav-logo" onclick="showPage('home')">
    <img src="data:image/png;base64," alt="NextPath System">
    <span>NextPath System</span>
  </a>
  <ul class="nav-links">
    <li><a href="#" onclick="showPage('home')" data-fr="Accueil" data-en="Home">Accueil</a></li>
    <li><a href="#" onclick="showPage('mission')" data-fr="Notre mission" data-en="Our mission">Notre mission</a></li>
    <li><a href="#" onclick="showPage('programmes')" data-fr="Nos programmes" data-en="Our programs">Nos programmes</a></li>
    <li><a href="#" onclick="showPage('videos')" data-fr="Vidéos" data-en="Videos">Vidéos</a></li>
    <li><a href="#" onclick="showPage('ressources')" data-fr="Ressources" data-en="Resources">Ressources</a></li>
    <li><a href="#" onclick="showPage('grace')" data-fr="À propos" data-en="About">À propos</a></li>
    <li><a href="#" onclick="showPage('contact')" data-fr="Contact" data-en="Contact">Contact</a></li>
    <li><a href="https://tally.so/r/ZjGJQB" target="_blank" class="nav-cta" data-fr="Rejoindre" data-en="Join">Rejoindre</a></li>
    <li>
      <div class="lang-switcher">
        <button class="lang-btn active" onclick="setLang('fr')">FR</button>
        <button class="lang-btn" onclick="setLang('en')">EN</button>
      </div>
    </li>
  </ul>
  <button class="hamburger" onclick="toggleMenu()" aria-label="Menu">
    <span></span><span></span><span></span>
  </button>
</nav>

<!-- MOBILE MENU -->
<div class="mobile-menu" id="mobile-menu">
  <a href="#" onclick="showPage('home');toggleMenu()">Accueil</a>
  <a href="#" onclick="showPage('mission');toggleMenu()">Notre mission</a>
  <a href="#" onclick="showPage('programmes');toggleMenu()">Nos programmes</a>
  <a href="#" onclick="showPage('videos');toggleMenu()">Vidéos</a>
  <a href="#" onclick="showPage('ressources');toggleMenu()">Ressources</a>
  <a href="#" onclick="showPage('grace');toggleMenu()">À propos</a>
  <a href="#" onclick="showPage('contact');toggleMenu()">Contact</a>
  <a href="https://tally.so/r/ZjGJQB" target="_blank" style="color:var(--gold);font-weight:700;">Rejoindre The Grace Effect</a>
  <div style="display:flex;gap:8px;padding:8px 0;">
    <button class="lang-btn active" onclick="setLang('fr')">FR</button>
    <button class="lang-btn" onclick="setLang('en')">EN</button>
  </div>
</div>

<!-- ══ PAGE ACCUEIL ══ -->
<div class="page active" id="page-home">
  <div class="hero">
    <div class="hero-left">
      <div class="hero-tag">NextPath System ·Pour la Jeunesse Africaine</div>
      <h1 data-fr="Tu as plus de valeur<br>que tu ne <em>le crois.</em>" data-en="You are worth more<br>than you <em>believe.</em>">Tu as plus de valeur<br>que tu ne <em>le crois.</em></h1>
      <p class="hero-sub" data-fr="Une plateforme dédiée à la jeunesse africaine — 15 à 25 ans — pour se connaître, explorer son potentiel et construire un chemin qui lui ressemble vraiment." data-en="A platform dedicated to African youth — 15 to 25 years old — to know themselves, explore their potential and build a path that truly fits them.">Une plateforme dédiée à la jeunesse africaine — 15 à 25 ans — pour se connaître, explorer son potentiel et construire un chemin qui lui ressemble vraiment.</p>
      <div class="hero-actions">
        <a href="https://tally.so/r/ZjGJQB" target="_blank" class="btn-primary" data-fr="Rejoindre The Grace Effect" data-en="Join The Grace Effect">Rejoindre The Grace Effect</a>
        <a href="#" onclick="showPage('programmes')" class="btn-secondary" data-fr="Nos programmes" data-en="Our programs">Nos programmes</a>
      </div>
      <div class="hero-quote">
        <p data-fr='"Un jeune qui se connaît ne se perd jamais."' data-en='"A young person who knows themselves never gets lost."'>"Un jeune qui se connaît ne se perd jamais."</p>
        <cite>— Grace Kabondo, Fondatrice</cite>
      </div>
    </div>
    <div class="hero-right">
      <div class="hero-right-content">
        <div class="line"></div>
        <h2 data-fr="Devenir la référence de la jeunesse africaine" data-en="Becoming the reference for African youth">Devenir la référence de la jeunesse africaine</h2>
        <div class="line"></div>
      </div>
    </div>
  </div>

  <!-- STATS -->
  <div class="stats-strip">
    <div class="stats-inner">
      <div class="stat-col">
        <span class="stat-num">500+</span>
        <span class="stat-lbl" data-fr="Jeunes visés d'ici 2028" data-en="Youth targeted by 2028">Jeunes visés d'ici 2028</span>
      </div>
      <div class="stat-col">
        <span class="stat-num">15–25</span>
        <span class="stat-lbl" data-fr="tranche d'âge prioritaire" data-en="priority age range">tranche d'âge prioritaire</span>
      </div>
      <div class="stat-col" style="text-align:center;">
        <span class="stat-num">4</span>
        <span class="stat-lbl" data-fr="Programmes en cours de création" data-en="Programs being created">Programmes en cours de création</span>
      </div>
    </div>
  </div>

  <!-- CYCLE NEXTPATH -->
  <div class="section">
    <div class="container">
      <div style="text-align:center;margin-bottom:56px;">
        <div class="section-tag" style="justify-content:center;"><span style="display:none"></span><span data-fr="Notre approche" data-en="Our approach">Notre approche</span></div>
        <h2 class="section-title" style="text-align:center;" data-fr="Le Cycle NextPath" data-en="The NextPath Cycle">Le Cycle NextPath</h2>
        <p class="section-body" style="margin:0 auto;text-align:center;" data-fr="Trois étapes concrètes pour transformer la découverte de soi en chemin d'action." data-en="Three concrete steps to transform self-discovery into a path of action.">Trois étapes concrètes pour transformer la découverte de soi en chemin d'action.</p>
      </div>
      <div class="pillars-grid">
        <div class="pillar-card">
          <div class="pillar-num">01</div>
          <div class="pillar-title" data-fr="Connais-toi" data-en="Know yourself">Connais-toi</div>
          <p class="pillar-text" data-fr="Découvre ta personnalité, tes forces naturelles et tes valeurs profondes. Ce que tu es naturellement — pas ce qu'on t'a dit d'être." data-en="Discover your personality, natural strengths and deep values. What you truly are — not what you've been told to be.">Découvre ta personnalité, tes forces naturelles et tes valeurs profondes. Ce que tu es naturellement — pas ce qu'on t'a dit d'être.</p>
        </div>
        <div class="pillar-card">
          <div class="pillar-num">02</div>
          <div class="pillar-title" data-fr="Libère-toi" data-en="Free yourself">Libère-toi</div>
          <p class="pillar-text" data-fr="Identifie ce qui te bloque — pression familiale, peur du jugement, croyances limitantes. Comprendre le frein, c'est déjà commencer à le dépasser." data-en="Identify what's holding you back — family pressure, fear of judgment, limiting beliefs. Understanding the block is already the beginning of overcoming it.">Identifie ce qui te bloque — pression familiale, peur du jugement, croyances limitantes. Comprendre le frein, c'est déjà commencer à le dépasser.</p>
        </div>
        <div class="pillar-card">
          <div class="pillar-num">03</div>
          <div class="pillar-title" data-fr="Construis-toi" data-en="Build yourself">Construis-toi</div>
          <p class="pillar-text" data-fr="Passe à l'action avec des outils concrets et une communauté qui avance avec toi. Pas juste de l'inspiration — des résultats réels." data-en="Take action with concrete tools and a community moving forward with you. Not just inspiration — real results.">Passe à l'action avec des outils concrets et une communauté qui avance avec toi. Pas juste de l'inspiration — des résultats réels.</p>
        </div>
      </div>
      <div style="text-align:center;margin-top:48px;">
        <a href="#" onclick="showPage('mission')" class="btn-secondary" data-fr="En savoir plus" data-en="Learn more">En savoir plus</a>
      </div>
    </div>
  </div>

  <!-- AVIS -->
  <div class="section section-alt">
    <div class="container">
      <div style="text-align:center;margin-bottom:48px;">
        <div class="section-tag" style="justify-content:center;"><span style="display:none"></span><span data-fr="Témoignages" data-en="Testimonials">Témoignages</span></div>
        <h2 class="section-title" style="text-align:center;" data-fr='Ce <qu'ils> disent de <em>NextPath</em>'data-en='What they say about <em>NextPath</em>'>Ce qu'ils disent de <em>NextPath</em></h2>
      </div>
      <div class="avis-grid" id="avis-grid">
        <div class="avis-card">
          <div class="stars">★★★★★</div>
          <p class="avis-text">"NextPath System m'a aidé à comprendre qui je suis vraiment. Pour la première fois, je sais vers quoi je veux aller."</p>
          <div class="avis-author"><div class="avis-avatar">K</div><div><div class="avis-name">Kevine M.</div><div class="avis-loc">Kinshasa, RDC</div></div></div>
        </div>
        <div class="avis-card">
          <div class="stars">★★★★★</div>
          <p class="avis-text">"Le contenu de Grace parle vraiment à notre réalité. C'est rare et précieux pour nous les jeunes congolais."</p>
          <div class="avis-author"><div class="avis-avatar">J</div><div><div class="avis-name">Junior T.</div><div class="avis-loc">Lubumbashi, RDC</div></div></div>
        </div>
        <div class="avis-card">
          <div class="stars">★★★★★</div>
          <p class="avis-text">"J'attendais ce type de contenu depuis longtemps. Grace explique des choses que j'aurais voulu entendre à 17 ans."</p>
          <div class="avis-author"><div class="avis-avatar">A</div><div><div class="avis-name">Astrid N.</div><div class="avis-loc">Bruxelles, Diaspora</div></div></div>
        </div>
      </div>
      <!-- FORMULAIRE AVIS -->
      <div style="background:var(--navy);border-radius:4px;padding:48px;max-width:680px;margin:0 auto;text-align:center;">
        <div style="font-size:28px;margin-bottom:14px;">✍️</div>
        <h3 style="font-family:'Playfair Display',serif;font-size:24px;color:var(--gold);margin-bottom:10px;" data-fr="Laisse ton avis" data-en="Leave your review">Laisse ton avis</h3>
        <p style="font-size:14px;color:rgba(245,240,232,0.7);margin-bottom:28px;line-height:1.7;" data-fr="Ton témoignage peut changer la trajectoire d'un autre jeune." data-en="Your testimonial can change another young person's path.">Ton témoignage peut changer la trajectoire d'un autre jeune.</p>
        <div style="display:flex;flex-direction:column;gap:12px;text-align:left;">
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
            <div>
              <label style="display:block;font-size:10px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:var(--gold);margin-bottom:6px;">Prénom *</label>
              <input type="text" id="avis-prenom" placeholder="Ex : Grace" style="width:100%;padding:11px 14px;border:1px solid rgba(201,168,76,0.3);border-radius:2px;background:rgba(255,255,255,0.05);color:var(--cream);font-family:'Montserrat',sans-serif;font-size:13px;outline:none;">
            </div>
            <div>
              <label style="display:block;font-size:10px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:var(--gold);margin-bottom:6px;">Ville & Pays *</label>
              <input type="text" id="avis-ville" placeholder="Ex : Kinshasa, RDC" style="width:100%;padding:11px 14px;border:1px solid rgba(201,168,76,0.3);border-radius:2px;background:rgba(255,255,255,0.05);color:var(--cream);font-family:'Montserrat',sans-serif;font-size:13px;outline:none;">
            </div>
          </div>
          <div>
            <label style="display:block;font-size:10px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:var(--gold);margin-bottom:6px;">Note *</label>
            <div id="star-rating" style="display:flex;gap:6px;">
              <span onclick="setRating(1)" style="font-size:26px;cursor:pointer;color:rgba(201,168,76,0.3);" data-star="1">★</span>
              <span onclick="setRating(2)" style="font-size:26px;cursor:pointer;color:rgba(201,168,76,0.3);" data-star="2">★</span>
              <span onclick="setRating(3)" style="font-size:26px;cursor:pointer;color:rgba(201,168,76,0.3);" data-star="3">★</span>
              <span onclick="setRating(4)" style="font-size:26px;cursor:pointer;color:rgba(201,168,76,0.3);" data-star="4">★</span>
              <span onclick="setRating(5)" style="font-size:26px;cursor:pointer;color:rgba(201,168,76,0.3);" data-star="5">★</span>
            </div>
          </div>
          <div>
            <label style="display:block;font-size:10px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:var(--gold);margin-bottom:6px;">Témoignage *</label>
            <textarea id="avis-texte" rows="4" placeholder="Partage ton expérience avec NextPath System..." style="width:100%;padding:11px 14px;border:1px solid rgba(201,168,76,0.3);border-radius:2px;background:rgba(255,255,255,0.05);color:var(--cream);font-family:'Montserrat',sans-serif;font-size:13px;outline:none;resize:vertical;"></textarea>
          </div>
          <div style="text-align:center;">
            <button onclick="soumettreAvis()" style="background:var(--gold);color:var(--navy);font-family:'Montserrat',sans-serif;font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;padding:14px 36px;border:none;border-radius:2px;cursor:pointer;">Envoyer mon témoignage</button>
          </div>
          <div id="avis-confirmation" style="display:none;text-align:center;padding:14px;background:rgba(201,168,76,0.1);border:1px solid rgba(201,168,76,0.3);border-radius:2px;">
            <p style="font-size:13px;color:var(--gold);font-weight:500;">🎉 Merci ! Grace lira ton témoignage personnellement.</p>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>

<!-- ══ PAGE MISSION ══ -->
<div class="page" id="page-mission">
  <div class="section" style="padding-top:80px;">
    <div class="container">
      <div class="section-tag" data-fr="Notre mission" data-en="Our mission">Notre mission</div>
      <h2 class="section-title" data-fr='Éveiller. Orienter. <em>Transformer.</em>' data-en='Awaken. Guide. <em>Transform.</em>'>Éveiller. Orienter. <em>Transformer.</em></h2>
      <div class="divider"></div>
      <p class="section-body" style="max-width:700px;" data-fr="La jeunesse africaine a un potentiel immense. Mais personne ne lui a appris à le voir, à le nommer, à le construire. NextPath System comble ce vide — avec du contenu, des outils concrets et un accompagnement réel." data-en="African youth has immense potential. But no one has taught them to see it, name it, build it. NextPath System fills this gap — with content, concrete tools and real support.">La jeunesse africaine a un potentiel immense. Mais personne ne lui a appris à le voir, à le nommer, à le construire. NextPath System comble ce vide — avec du contenu, des outils concrets et un accompagnement réel.</p>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:24px;margin-top:64px;">
        <div style="padding:32px;background:var(--light-bg);border-radius:4px;border-top:3px solid var(--gold);">
          <div style="font-family:'Playfair Display',serif;font-size:26px;font-weight:700;color:var(--gold);margin-bottom:8px;" data-fr="Éveiller" data-en="Awaken">Éveiller</div>
          <p style="font-size:14px;color:var(--text-mid);line-height:1.8;" data-fr="Faire prendre conscience à chaque jeune de sa valeur et de son potentiel — avant même de parler de carrière." data-en="Making every young person aware of their value and potential — before even talking about career.">Faire prendre conscience à chaque jeune de sa valeur et de son potentiel — avant même de parler de carrière.</p>
        </div>
        <div style="padding:32px;background:var(--navy);border-radius:4px;border-top:3px solid var(--gold);">
          <div style="font-family:'Playfair Display',serif;font-size:26px;font-weight:700;color:var(--gold);margin-bottom:8px;" data-fr="Orienter" data-en="Guide">Orienter</div>
          <p style="font-size:14px;color:rgba(245,240,232,0.75);line-height:1.8;" data-fr="Donner des outils concrets pour identifier ses forces et les directions qui correspondent vraiment à qui on est." data-en="Providing concrete tools to identify strengths and directions that truly match who one is.">Donner des outils concrets pour identifier ses forces et les directions qui correspondent vraiment à qui on est.</p>
        </div>
        <div style="padding:32px;background:var(--light-bg);border-radius:4px;border-top:3px solid var(--gold);">
          <div style="font-family:'Playfair Display',serif;font-size:26px;font-weight:700;color:var(--gold);margin-bottom:8px;" data-fr="Transformer" data-en="Transform">Transformer</div>
          <p style="font-size:14px;color:var(--text-mid);line-height:1.8;" data-fr="Accompagner la mise en action — avec un plan clair, une communauté engagée et un suivi qui crée des résultats." data-en="Supporting action — with a clear plan, engaged community and follow-up that creates real results.">Accompagner la mise en action — avec un plan clair, une communauté engagée et un suivi qui crée des résultats.</p>
        </div>
      </div>
    </div>
  </div>
</div>

<!-- ══ PAGE NOS PROGRAMMES ══ -->
<div class="page" id="page-programmes">
  <div class="section" style="padding-top:80px;">
    <div class="container">
      <div class="section-tag" data-fr="Nos programmes" data-en="Our programs">Nos programmes</div>
      <h2 class="section-title" data-fr='Deux outils pour <em>te révéler.</em>' data-en='Two tools to <em>reveal you.</em>'>Deux outils pour <em>te révéler.</em></h2>
      <div class="divider"></div>
      <div class="prog-grid">
        <div class="prog-card" onclick="showProgramme('grace')">
          <span class="prog-icon">🔥</span>
          <div class="prog-badge" data-fr="Programme · 4 semaines" data-en="Program · 4 weeks">Programme · 4 semaines</div>
          <div class="prog-name">The Grace Effect</div>
          <p class="prog-desc" data-fr="4 semaines pour te connaître, comprendre ce qui te bloque et construire ton chemin. En ligne, en groupe, avec Grace. Première cohorte totalement gratuite." data-en="4 weeks to know yourself, understand what's holding you back and build your path. Online, in a group, with Grace. First cohort completely free.">4 semaines pour te connaître, comprendre ce qui te bloque et construire ton chemin. En ligne, en groupe, avec Grace. Première cohorte totalement gratuite.</p>
          <a href="https://tally.so/r/ZjGJQB" target="_blank" class="btn-primary" onclick="event.stopPropagation()" data-fr="Réserver ma place gratuite" data-en="Reserve my free spot">Réserver ma place gratuite</a>
        </div>
        <div class="prog-card" onclick="showProgramme('test')">
          <span class="prog-icon">🧠</span>
          <div class="prog-badge" data-fr="Outil gratuit · 10 min" data-en="Free tool · 10 min">Outil gratuit · 10 min</div>
          <div class="prog-name">NextPath Test</div>
          <p class="prog-desc" data-fr="13 questions pour révéler ton profil de personnalité, tes forces naturelles, tes blocages et les directions qui te correspondent. Gratuit, sans inscription." data-en="20 questions to reveal your personality profile, natural strengths, blocks and directions that suit you. Free, no registration.">13 questions pour révéler ton profil de personnalité, tes forces naturelles, tes blocages et les directions qui te correspondent. Gratuit, sans inscription.</p>
          <a href="https://docs.google.com/forms/d/e/1FAIpQLSdvA440a9h65ZcFT4XXJUoPJBelQbqrmUgU27X1pDnX6I-XkQ/viewform?usp=header" target="_blank" class="btn-secondary" onclick="event.stopPropagation()" data-fr="Passer le test" data-en="Take the test">Passer le test</a>
        </div>
      </div>

      <!-- DETAIL GRACE EFFECT -->
      <div id="detail-grace" style="display:none;margin-top:64px;">
        <h3 style="font-family:'Playfair Display',serif;font-size:28px;font-weight:700;color:var(--navy);margin-bottom:32px;" data-fr="The Grace Effect — Détail du programme" data-en="The Grace Effect — Program details">The Grace Effect — Détail du programme</h3>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:48px;">
          <div>
            <div class="module-list">
              <div class="module-item"><span class="module-week">Semaine 1</span><div><h4 data-fr="Qui es-tu vraiment ?" data-en="Who are you really?">Qui es-tu vraiment ?</h4><p data-fr="Profil NextPath, forces naturelles, valeurs fondamentales." data-en="NextPath profile, natural strengths, core values.">Profil NextPath, forces naturelles, valeurs fondamentales.</p></div></div>
              <div class="module-item"><span class="module-week">Semaine 2</span><div><h4 data-fr="Qu'est-ce qui te bloque ?" data-en="What's holding you back?">Qu'est-ce qui te bloque ?</h4><p data-fr="Identification des blocages et outils pour s'en libérer." data-en="Identifying blocks and tools to overcome them.">Identification des blocages et outils pour s'en libérer.</p></div></div>
              <div class="module-item"><span class="module-week">Semaine 3</span><div><h4 data-fr="Comment construire ton chemin ?" data-en="How to build your path?">Comment construire ton chemin ?</h4><p data-fr="Méthode NextPath pour connecter tes forces à un chemin concret." data-en="NextPath method to connect your strengths to a concrete path.">Méthode NextPath pour connecter tes forces à un chemin concret.</p></div></div>
              <div class="module-item"><span class="module-week">Semaine 4</span><div><h4 data-fr="Ton plan pour avancer" data-en="Your plan to move forward">Ton plan pour avancer</h4><p data-fr="Premières actions concrètes ancrées dans qui tu es." data-en="First concrete actions rooted in who you are.">Premières actions concrètes ancrées dans qui tu es.</p></div></div>
            </div>
          </div>
          <div>
            <div>
              <div class="info-row"><span>Format</span><span data-fr="100% en ligne · Sessions de groupe" data-en="100% online · Group sessions">100% en ligne · Sessions de groupe</span></div>
              <div class="info-row"><span data-fr="Durée" data-en="Duration">Durée</span><span data-fr="4 semaines · 90 min / session" data-en="4 weeks · 90 min / session">4 semaines · 90 min / session</span></div>
              <div class="info-row"><span data-fr="Places" data-en="Spots">Places</span><span>15 max</span></div>
              <div class="info-row"><span data-fr="Âge" data-en="Age">Âge</span><span>15 à 25 ans</span></div>
              <div class="info-row"><span>Plateforme</span><span>Zoom + WhatsApp</span></div>
            </div>
            <div class="programme-cta" style="margin-top:24px;">
              <div class="gratuit-badge">🎉 100% Gratuit</div>
              <h3 data-fr="Rejoins The Grace Effect" data-en="Join The Grace Effect">Rejoins The Grace Effect</h3>
              <p data-fr="15 places disponibles. Ne laisse pas ta place à quelqu'un d'autre." data-en="15 spots available. Don't let someone else take your spot.">15 places disponibles. Ne laisse pas ta place à quelqu'un d'autre.</p>
              <a href="https://tally.so/r/ZjGJQB" target="_blank" class="btn-gold" data-fr="Réserver ma place" data-en="Reserve my spot">Réserver ma place</a>
            </div>
          </div>
        </div>
      </div>

      <!-- DETAIL TEST -->
      <div id="detail-test" style="display:none;margin-top:64px;">
        <h3 style="font-family:'Playfair Display',serif;font-size:28px;font-weight:700;color:var(--navy);margin-bottom:32px;" data-fr="NextPath Test — Découvre ton profil" data-en="NextPath Test — Discover your profile">NextPath Test — Découvre ton profil</h3>
        <div class="profiles-grid">
          <div class="profile-card"><span class="profile-icon">🧠</span><div class="profile-name">L'Éclaireur</div><p class="profile-desc" data-fr="Sciences, recherche, enseignement, droit." data-en="Sciences, research, teaching, law.">Sciences, recherche, enseignement, droit.</p></div>
          <div class="profile-card"><span class="profile-icon">🌐</span><div class="profile-name">Le Connecteur</div><p class="profile-desc" data-fr="Médias, diplomatie, entrepreneuriat social." data-en="Media, diplomacy, social entrepreneurship.">Médias, diplomatie, entrepreneuriat social.</p></div>
          <div class="profile-card"><span class="profile-icon">🏗️</span><div class="profile-name">Le Bâtisseur</div><p class="profile-desc" data-fr="Entrepreneuriat, management, commerce." data-en="Entrepreneurship, management, business.">Entrepreneuriat, management, commerce.</p></div>
          <div class="profile-card"><span class="profile-icon">🌱</span><div class="profile-name">Le Semeur</div><p class="profile-desc" data-fr="Éducation, santé, social, arts." data-en="Education, health, social, arts.">Éducation, santé, social, arts.</p></div>
        </div>
        <div style="text-align:center;margin-top:32px;">
          <a href="https://docs.google.com/forms/d/e/1FAIpQLSdvA440a9h65ZcFT4XXJUoPJBelQbqrmUgU27X1pDnX6I-XkQ/viewform?usp=header" target="_blank" class="btn-gold" data-fr="Passer le NextPath Test — Gratuit" data-en="Take the NextPath Test — Free">Passer le NextPath Test — Gratuit</a>
        </div>
      </div>

      <!-- DEVENIR MEMBRE -->
      <div style="margin-top:64px;background:var(--light-bg);border-radius:4px;padding:48px;display:grid;grid-template-columns:1fr 1fr;gap:48px;align-items:center;">
        <div>
          <div class="section-tag" data-fr="Communauté" data-en="Community">Communauté</div>
          <h3 class="section-title" style="font-size:32px;" data-fr='Devenir <em>membre</em>' data-en='Become a <em>member</em>'>Devenir <em>membre</em></h3>
          <div class="divider"></div>
          <p style="font-size:15px;color:var(--text-mid);line-height:1.85;margin-bottom:24px;" data-fr="Rejoins la communauté NextPath System. Accède aux ressources, aux événements en ligne et aux échanges. C'est gratuit." data-en="Join the NextPath System community. Access resources, online events and discussions. It's free.">Rejoins la communauté NextPath System. Accède aux ressources, aux événements en ligne et aux échanges. C'est gratuit.</p>
          <div class="benefit-item"><div class="benefit-dot">✦</div><span data-fr="Accès aux lives et webinaires exclusifs" data-en="Access to exclusive lives and webinars">Accès aux lives et webinaires exclusifs</span></div>
          <div class="benefit-item"><div class="benefit-dot">✦</div><span data-fr="Groupe WhatsApp de la communauté" data-en="Community WhatsApp group">Groupe WhatsApp de la communauté</span></div>
          <div class="benefit-item"><div class="benefit-dot">✦</div><span data-fr="Ressources et outils NextPath gratuits" data-en="Free NextPath resources and tools">Ressources et outils NextPath gratuits</span></div>
          <div class="benefit-item"><div class="benefit-dot">✦</div><span data-fr="Priorité pour les prochaines cohortes" data-en="Priority for upcoming cohorts">Priorité pour les prochaines cohortes</span></div>
        </div>
        <div style="background:#fff;border-radius:4px;padding:40px;text-align:center;border:1px solid var(--border);">
          <div style="font-size:40px;margin-bottom:16px;">🌍</div>
          <h4 style="font-family:'Playfair Display',serif;font-size:20px;color:var(--navy);margin-bottom:10px;" data-fr="Rejoins le mouvement" data-en="Join the movement">Rejoins le mouvement</h4>
          <p style="font-size:14px;color:var(--text-mid);margin-bottom:24px;line-height:1.7;" data-fr="Fais partie des premiers membres de la communauté NextPath System." data-en="Be among the first members of the NextPath System community.">Fais partie des premiers membres de la communauté NextPath System.</p>
          <a href="https://tally.so/r/81Jey5" target="_blank" class="btn-primary" style="width:100%;justify-content:center;" data-fr="Devenir membre — Gratuit" data-en="Become a member — Free">Devenir membre — Gratuit</a>
          <p style="font-size:11px;color:var(--text-light);margin-top:12px;" data-fr="100% gratuit · Sans engagement" data-en="100% free · No commitment">100% gratuit · Sans engagement</p>
        </div>
      </div>
    </div>
  </div>
</div>

<!-- ══ PAGE VIDÉOS ══ -->
<div class="page" id="page-videos">
  <div class="section" style="padding-top:80px;">
    <div class="container">
      <div class="section-tag" data-fr="Nos dernières vidéos" data-en="Our latest videos">Nos dernières vidéos</div>
      <h2 class="section-title" data-fr='Contenu pour<em>te stimuler'.</em>' data-en='Content to <em>awaken you.</em>'>Contenu pour <em>t'éveiller.</em></h2>
      <div class="divider"></div>
      <p class="section-body" style="margin-bottom:40px;" data-fr="Retrouve toutes nos vidéos sur YouTube. Des contenus concrets pour te connaître, te libérer et construire ton chemin." data-en="Find all our videos on YouTube. Concrete content to know yourself, free yourself and build your path.">Retrouve toutes nos vidéos sur YouTube. Des contenus concrets pour te connaître, te libérer et construire ton chemin.</p>

      <div class="yt-grid">
        <a href="https://www.youtube.com/@TheNextPathSystem" target="_blank" class="yt-card">
          <div class="yt-thumb">
            <div class="yt-thumb-bg"></div>
            <div style="position:relative;z-index:1;text-align:center;padding:20px;">
              <div class="yt-play" style="margin:0 auto 12px;"></div>
              <div style="font-size:12px;color:rgba(245,240,232,0.7);letter-spacing:.06em;text-transform:uppercase;">The Waken Series</div>
            </div>
          </div>
          <div class="yt-info">
            <div class="yt-title" data-fr="Ton diplôme d'État peut te rendre utile si tu sais l'utiliser" data-en="Your high school diploma can make you useful if you know how to use it">Ton diplôme d'État peut te rendre utile si tu sais l'utiliser</div>
            <div class="yt-meta">NextPath System · YouTube</div>
          </div>
        </a>
        <a href="https://www.youtube.com/@TheNextPathSystem" target="_blank" class="yt-card">
          <div class="yt-thumb">
            <div class="yt-thumb-bg"></div>
            <div style="position:relative;z-index:1;text-align:center;padding:20px;">
              <div class="yt-play" style="margin:0 auto 12px;"></div>
              <div style="font-size:12px;color:rgba(245,240,232,0.7);letter-spacing:.06em;text-transform:uppercase;">The Waken Series</div>
            </div>
          </div>
          <div class="yt-info">
            <div class="yt-title" data-fr="On nous a tout appris sauf à construire notre avenir" data-en="We were taught everything except how to build our future">On nous a tout appris sauf à construire notre avenir</div>
            <div class="yt-meta">NextPath System · YouTube</div>
          </div>
        </a>
        <a href="https://www.youtube.com/@TheNextPathSystem" target="_blank" class="yt-card">
          <div class="yt-thumb">
            <div class="yt-thumb-bg"></div>
            <div style="position:relative;z-index:1;text-align:center;padding:20px;">
              <div class="yt-play" style="margin:0 auto 12px;"></div>
              <div style="font-size:12px;color:rgba(245,240,232,0.7);letter-spacing:.06em;text-transform:uppercase;">The Waken Series</div>
            </div>
          </div>
          <div class="yt-info">
            <div class="yt-title" data-fr="Comment construire son identité et augmenter sa valeur ?" data-en="How to build your identity and increase your value?">Comment construire son identité et augmenter sa valeur ?</div>
            <div class="yt-meta">NextPath System · YouTube</div>
          </div>
        </a>
      </div>

      <div style="text-align:center;margin-top:48px;">
        <a href="https://www.youtube.com/@TheNextPathSystem" target="_blank" class="btn-primary" data-fr="Voir toutes les vidéos sur YouTube" data-en="See all videos on YouTube">Voir toutes les vidéos sur YouTube</a>
      </div>
    </div>
  </div>
</div>

<!-- ══ PAGE RESSOURCES ══ -->
<div class="page" id="page-ressources">
  <div class="section" style="padding-top:80px;">
    <div class="container">
      <div class="section-tag" data-fr="Ressources gratuites" data-en="Free resources">Ressources gratuites</div>
      <h2 class="section-title" data-fr='Guides & <em>outils</em>' data-en='Guides & <em>tools</em>'>Guides & <em>outils</em></h2>
      <div class="divider"></div>
      <p class="section-body" style="margin-bottom:40px;" data-fr="Des ressources concrètes pour avancer — guides PDF, outils d'orientation, exercices pratiques. Télécharge gratuitement." data-en="Concrete resources to move forward — PDF guides, orientation tools, practical exercises. Download for free.">Des ressources concrètes pour avancer — guides PDF, outils d'orientation, exercices pratiques. Télécharge gratuitement.</p>

      <div class="pdf-grid">
        <div class="pdf-card" style="cursor:default;opacity:.6;">
          <div class="pdf-icon">📄</div>
          <div>
            <div class="pdf-title" data-fr="Guide : Comment identifier ton potentiel" data-en="Guide: How to identify your potential">Guide : Comment identifier ton potentiel</div>
            <div class="pdf-desc" data-fr="3 étapes concrètes pour découvrir tes forces naturelles et les connecter à un chemin réel." data-en="3 concrete steps to discover your natural strengths and connect them to a real path.">3 étapes concrètes pour découvrir tes forces naturelles et les connecter à un chemin réel.</div>
            <div class="pdf-dl" style="opacity:.5;" data-fr="Bientôt disponible" data-en="Coming soon">Bientôt disponible</div>
          </div>
        </div>
        <div class="pdf-card" style="cursor:default;opacity:.6;">
          <div class="pdf-icon">📄</div>
          <div>
            <div class="pdf-title" data-fr="Guide : Construire ton identité" data-en="Guide: Building your identity">Guide : Construire ton identité</div>
            <div class="pdf-desc" data-fr="Comment définir tes valeurs, tes forces et ton chemin en 5 exercices pratiques." data-en="How to define your values, strengths and path in 5 practical exercises.">Comment définir tes valeurs, tes forces et ton chemin en 5 exercices pratiques.</div>
            <div class="pdf-dl" style="opacity:.5;" data-fr="Bientôt disponible" data-en="Coming soon">Bientôt disponible</div>
          </div>
        </div>
        <div class="pdf-card" style="cursor:default;opacity:.6;">
          <div class="pdf-icon">📄</div>
          <div>
            <div class="pdf-title" data-fr="Guide : Orientation après le diplôme d'État" data-en="Guide: Orientation after high school diploma">Guide : Orientation après le diplôme d'État</div>
            <div class="pdf-desc" data-fr="Les questions à se poser, les pistes à explorer, les erreurs à éviter." data-en="Questions to ask, paths to explore, mistakes to avoid.">Les questions à se poser, les pistes à explorer, les erreurs à éviter.</div>
            <div class="pdf-dl" style="opacity:.5;" data-fr="Bientôt disponible" data-en="Coming soon">Bientôt disponible</div>
          </div>
        </div>
      </div>

      <div style="margin-top:48px;background:var(--navy);border-radius:4px;padding:48px;text-align:center;">
        <div style="font-size:32px;margin-bottom:14px;">📬</div>
        <h3 style="font-family:'Playfair Display',serif;font-size:24px;color:var(--gold);margin-bottom:10px;" data-fr="Sois le premier à recevoir les guides" data-en="Be the first to receive the guides">Sois le premier à recevoir les guides</h3>
        <p style="font-size:14px;color:rgba(245,240,232,0.7);max-width:480px;margin:0 auto 28px;line-height:1.7;" data-fr="Les ressources sont en cours de création. Rejoins la communauté pour les recevoir dès leur sortie." data-en="Resources are being created. Join the community to receive them as soon as they're released.">Les ressources sont en cours de création. Rejoins la communauté pour les recevoir dès leur sortie.</p>
        <a href="https://tally.so/r/81Jey5" target="_blank" class="btn-gold" data-fr="Rejoindre la communauté" data-en="Join the community">Rejoindre la communauté</a>
      </div>
    </div>
  </div>
</div>

<!-- ══ PAGE À PROPOS ══ -->
<div class="page" id="page-grace">
  <div style="background:var(--navy);padding:80px 80px 0;display:grid;grid-template-columns:1fr 1fr;gap:0;align-items:end;min-height:380px;overflow:hidden;">
    <div style="padding-bottom:60px;">
      <div style="font-size:11px;font-weight:600;letter-spacing:.18em;text-transform:uppercase;color:var(--gold);margin-bottom:16px;display:flex;align-items:center;gap:10px;"><span style="display:block;width:24px;height:1.5px;background:var(--gold);"></span><span data-fr="À propos" data-en="About">À propos</span></div>
      <h1 style="font-family:'Playfair Display',serif;font-size:clamp(36px,5vw,56px);font-weight:900;color:var(--cream);line-height:1.1;margin-bottom:18px;">Grace<br><em style="color:var(--gold);">Kabondo</em></h1>
      <p style="font-size:15px;color:rgba(245,240,232,0.65);line-height:1.7;max-width:440px;margin-bottom:28px;" data-fr="Fondatrice de NextPath System " data-en="Founder of NextPath System">Fondatrice de NextPath System </p>
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <span style="font-size:10px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;padding:6px 14px;border:1px solid rgba(201,168,76,0.4);border-radius:2px;color:var(--gold);" data-fr="Master Sciences du Médicament" data-en="MSc Drug Sciences">Master Sciences du Médicament</span>
        <span style="font-size:10px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;padding:6px 14px;border:1px solid rgba(201,168,76,0.4);border-radius:2px;color:var(--gold);">Nantes Université</span>
      </div>
    </div>
    <div style="position:relative;height:380px;overflow:hidden;border-radius:4px 4px 0 0;">
      <img src="data:image/jpeg;base64," alt="Grace Kabondo" style="width:100%;height:100%;object-fit:cover;object-position:center top;">
      <div style="position:absolute;inset:0;background:linear-gradient(to top,rgba(13,27,62,0.3) 0%,transparent 60%);"></div>
    </div>
  </div>
  <div class="section">
    <div class="container">
      <div class="grace-grid">
        <div>
          <div class="section-tag" data-fr="Mon histoire" data-en="My story">Mon histoire</div>
          <h2 class="section-title" style="font-size:34px;" data-fr='Je suis passée<br>par là, <em>moi aussi.</em>' data-en='I've been there,<br><em>too.Je suis passée<br>par là, <em>moi aussi.</em></h2>
          <div class="divider"></div>
          <p style="font-size:15px;color:var(--text-mid);line-height:1.85;margin-bottom:18px;" data-fr="J'ai fait mon école secondaire à Lubumbashi, en RDC — au cœur du système, de la pression, des choix imposés et des questions sans réponses." data-en="I did my secondary school in Lubumbashi, DRC — at the heart of the system, pressure, imposed choices and unanswered questions.">J'ai fait mon école secondaire à Lubumbashi, en RDC — au cœur du système, de la pression, des choix imposés et des questions sans réponses.</p>
          <p style="font-size:15px;color:var(--text-mid);line-height:1.85;margin-bottom:18px;" data-fr="Puis j'ai poursuivi mes études supérieures en France, où j'ai obtenu une Licence en Chimie-Biologie à Nantes Université, avant de poursuivre un Master en Sciences du Médicament." data-en="Then I pursued higher education in France, where I obtained a Bachelor's in Chemistry-Biology at Nantes University, before pursuing a Master's in Drug Sciences.">Puis j'ai poursuivi mes études supérieures en France, où j'ai obtenu une Licence en Chimie-Biologie à Nantes Université, avant de poursuivre un Master en Sciences du Médicament.</p>
          <p style="font-size:15px;color:var(--text-mid);line-height:1.85;margin-bottom:28px;" data-fr="Ce parcours entre deux mondes m'a révélé une vérité : personne ne m'avait appris à me connaître. À transformer ce que j'avais en quelque chose de concret." data-en="This journey between two worlds revealed a truth: no one had taught me to know myself. To transform what I had into something concrete.">Ce parcours entre deux mondes m'a révélé une vérité : personne ne m'avait appris à me connaître. À transformer ce que j'avais en quelque chose de concret.</p>
          <div class="tagline-box">
            <p data-fr='"NextPath System c'est ce que j'aurais voulu avoir à 17 ans. C'est ce que je construis aujourd'hui — pour toi."' data-en='"NextPath System is what I wish I had at 17. It's what I'm building today — for you."'>"NextPath System c'est ce que j'aurais voulu avoir à 17 ans. C'est ce que je construis aujourd'hui — pour toi."</p>
          </div>
        </div>
        <div>
          <div style="display:flex;flex-direction:column;gap:0;border:1px solid var(--border);border-radius:4px;overflow:hidden;">
            <div style="display:flex;gap:14px;padding:14px 18px;border-bottom:1px solid var(--border);">
              <span style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--gold);min-width:80px;margin-top:2px;" data-fr="Origine" data-en="Origin">Origine</span>
              <span style="font-size:14px;color:var(--text-dark);" data-fr="Lubumbashi, RDC" data-en="Lubumbashi, DRC">Lubumbashi, RDC</span>
            </div>
            <div style="display:flex;gap:14px;padding:14px 18px;border-bottom:1px solid var(--border);">
              <span style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--gold);min-width:80px;margin-top:2px;" data-fr="Basée" data-en="Based">Basée</span>
              <span style="font-size:14px;color:var(--text-dark);">France</span>
            </div>
            <div style="display:flex;gap:14px;padding:14px 18px;border-bottom:1px solid var(--border);">
              <span style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--gold);min-width:80px;margin-top:2px;" data-fr="Formation" data-en="Education">Formation</span>
              <span style="font-size:14px;color:var(--text-dark);line-height:1.6;" data-fr="Licence Chimie-Biologie · Nantes Université<br>Master Sciences du Médicament" data-en="Bachelor Chemistry-Biology · Nantes University<br>MSc Drug Sciences">Licence Chimie-Biologie<br>Master Sciences du Médicament</span>
            </div>
            <div style="display:flex;gap:14px;padding:14px 18px;border-bottom:1px solid var(--border);">
              <span style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--gold);min-width:80px;margin-top:2px;" data-fr="Rôle" data-en="Role">Rôle</span>
              <span style="font-size:14px;color:var(--text-dark);" data-fr="Fondatrice & CEO · NextPath System" data-en="Founder & CEO · NextPath System">Fondatrice· NextPath System</span>
            </div>
            <div style="display:flex;gap:14px;padding:14px 18px;">
              <span style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--gold);min-width:80px;margin-top:2px;">Contact</span>
              <span style="font-size:14px;color:var(--text-dark);">nextpathsystem@gmail.com</span>
            </div>
          </div>
          <div style="margin-top:24px;background:var(--navy);border-radius:4px;padding:40px;text-align:center;">
            <p style="font-family:'Playfair Display',serif;font-size:18px;font-style:italic;color:var(--cream);line-height:1.65;margin-bottom:16px;" data-fr='"Un jeune qui se connaît ne se perd jamais."' data-en='"A young person who knows themselves never gets lost."'>"Un jeune qui se connaît ne se perd jamais."</p>
            <cite style="font-size:11px;letter-spacing:.1em;text-transform:uppercase;color:var(--gold);">— Grace Kabondo</cite>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>

<!-- ══ PAGE CONTACT ══ -->
<div class="page" id="page-contact">
  <div class="section" style="padding-top:80px;">
    <div class="container">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:80px;align-items:start;">
        <div>
          <div class="section-tag" data-fr="Nous contacter" data-en="Contact us">Nous contacter</div>
          <h2 class="section-title" data-fr='Ensemble,<br>on <em>construit</em><br>une génération.' data-en='Together,<br>we <em>build</em><br>a generation.'>Ensemble,<br>on <em>construit</em><br>une génération.</h2>
          <div class="divider"></div>
          <a href="https://instagram.com/nextpath.system" target="_blank" class="social-link"><span class="social-platform">Instagram</span><span class="social-handle">@nextpath.system</span></a>
          <a href="https://www.youtube.com/@TheNextPathSystem" target="_blank" class="social-link"><span class="social-platform">YouTube</span><span class="social-handle">The NextPath System</span></a>
          <a href="https://tiktok.com/@nextpath.system" target="_blank" class="social-link"><span class="social-platform">TikTok</span><span class="social-handle">@nextpath.system</span></a>
          <a href="https://wa.me/33766131603" target="_blank" class="social-link"><span class="social-platform">WhatsApp</span><span class="social-handle">+33 7 66 13 16 03</span></a>
          <a href="mailto:nextpathsystem@gmail.com" class="social-link"><span class="social-platform">Email</span><span class="social-handle">nextpathsystem@gmail.com</span></a>
        </div>
        <div style="background:var(--light-bg);border-radius:4px;padding:40px;">
          <h3 style="font-family:'Playfair Display',serif;font-size:22px;color:var(--navy);margin-bottom:8px;" data-fr="Rejoins The Grace Effect" data-en="Join The Grace Effect">Rejoins The Grace Effect</h3>
          <p style="font-size:14px;color:var(--text-mid);margin-bottom:24px;line-height:1.7;" data-fr="15 places. Totalement gratuit. Grace te contacte sous 48h." data-en="15 spots. Completely free. Grace will contact you within 48h.">15 places. Totalement gratuit. Grace te contacte sous 48h.</p>
          <a href="https://tally.so/r/ZjGJQB" target="_blank" class="btn-primary" style="width:100%;justify-content:center;margin-bottom:14px;" data-fr="Rejoindre The Grace Effect" data-en="Join The Grace Effect">Rejoindre The Grace Effect</a>
          <div style="text-align:center;margin:12px 0;font-size:12px;color:var(--text-light);" data-fr="ou" data-en="or">ou</div>
          <a href="https://tally.so/r/81Jey5" target="_blank" class="btn-secondary" style="width:100%;justify-content:center;" data-fr="Devenir membre de la communauté" data-en="Become a community member">Devenir membre de la communauté</a>
        </div>
      </div>
    </div>
  </div>
</div>

<!-- ══ FOOTER ══ -->
<footer>
  <div class="footer-logo">NextPath System</div>
  <div class="footer-links">
    <a href="#" onclick="showPage('home')" data-fr="Accueil" data-en="Home">Accueil</a>
    <a href="#" onclick="showPage('mission')" data-fr="Mission" data-en="Mission">Mission</a>
    <a href="#" onclick="showPage('programmes')" data-fr="Programmes" data-en="Programs">Programmes</a>
    <a href="#" onclick="showPage('videos')" data-fr="Vidéos" data-en="Videos">Vidéos</a>
    <a href="#" onclick="showPage('ressources')" data-fr="Ressources" data-en="Resources">Ressources</a>
    <a href="#" onclick="showPage('grace')" data-fr="À propos" data-en="About">À propos</a>
    <a href="#" onclick="showPage('contact')" data-fr="Contact" data-en="Contact">Contact</a>
  </div>
  <div class="footer-copy">© 2026 Grace Kabondo · NextPath System</div>
</footer>

<script>
// ── NAVIGATION ──
function showPage(pageId) {
  document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
  var t = document.getElementById('page-' + pageId);
  if (t) { t.classList.add('active'); window.scrollTo(0,0); }
  document.getElementById('mobile-menu').classList.remove('open');
}

function toggleMenu() {
  document.getElementById('mobile-menu').classList.toggle('open');
}

// ── PROGRAMMES ──
function showProgramme(which) {
  document.getElementById('detail-grace').style.display = 'none';
  document.getElementById('detail-test').style.display = 'none';
  var el = document.getElementById('detail-' + which);
  if (el) {
    el.style.display = 'block';
    el.scrollIntoView({behavior:'smooth', block:'start'});
  }
}

// ── AVIS ──
var currentRating = 0;
function setRating(n) {
  currentRating = n;
  document.querySelectorAll('#star-rating span').forEach(function(s) {
    s.style.color = parseInt(s.dataset.star) <= n ? 'var(--gold)' : 'rgba(201,168,76,0.3)';
  });
}
function soumettreAvis() {
  var prenom = document.getElementById('avis-prenom').value.trim();
  var ville = document.getElementById('avis-ville').value.trim();
  var texte = document.getElementById('avis-texte').value.trim();
  if (!prenom || !ville || !texte || currentRating === 0) {
    alert('Merci de remplir tous les champs et de donner une note.'); return;
  }
  var initiale = prenom.charAt(0).toUpperCase();
  var etoiles = '';
  for (var i = 0; i < currentRating; i++) etoiles += '★';
  var card = document.createElement('div');
  card.className = 'avis-card';
  card.innerHTML = '<div class="stars">' + etoiles + '</div><p class="avis-text">"' + texte + '"</p><div class="avis-author"><div class="avis-avatar">' + initiale + '</div><div><div class="avis-name">' + prenom + '</div><div class="avis-loc">' + ville + '</div></div></div>';
  document.getElementById('avis-grid').appendChild(card);
  document.getElementById('avis-confirmation').style.display = 'block';
  document.getElementById('avis-prenom').value = '';
  document.getElementById('avis-ville').value = '';
  document.getElementById('avis-texte').value = '';
  currentRating = 0;
  document.querySelectorAll('#star-rating span').forEach(function(s) {
    s.style.color = 'rgba(201,168,76,0.3)';
  });
}

// ── LANGUE ──
var currentLang = 'fr';
function setLang(lang) {
  currentLang = lang;
  document.querySelectorAll('.lang-btn').forEach(function(b) {
    b.classList.toggle('active', b.textContent === lang.toUpperCase());
  });
  document.querySelectorAll('[data-' + lang + ']').forEach(function(el) {
    var val = el.getAttribute('data-' + lang);
    if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
      el.placeholder = val;
    } else {
      el.innerHTML = val;
    }
  });
}
</script>
</body>
</html>
